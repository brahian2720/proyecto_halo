...
const baseUrl = "https://www.haloapi.com/profile/h5/profiles/{player}/appearance";
let halos = document.getElementById('halos');
let prev = document.getElementById('prev')
let next = document.getElementById('next')

makeRequest(baseUrl, "gethalos(data)");
...
function gethalos(data) {
    prev.dataset.prev = data.previous;
    next.dataset.next = data.next;

    data.results.forEach(halos => {
        makeRequest(halos.url, "createCard(data)")
    });
}
...
function showDetail(name, img, stats) {
    Swal.fire({
        ...
        backdrop: `
            rgba(0,0,123,0.4)
            url("../assets/img/icegif-1476.gif")
            left top
            no-repeat
            `
    });
